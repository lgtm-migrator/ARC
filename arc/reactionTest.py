...
class TestARCReaction(unittest.TestCase):
    ...
    def test_rxn_family(self):
        ...
        rxn_3 = ARCReaction(r_species=[ARCSpecies(label='C2H5O3', smiles='CC(O)O[O]')],
                            p_species=[ARCSpecies(label='C2H4O', smiles='CC=O'),
                                       ARCSpecies(label='HO2', smiles='O[O]')])
        rxn_3.check_attributes()
        rxn_3.determine_family(rmg_database=self.rmgdb)
        self.assertEqual(rxn_3.family.label, 'HO2_Elimination_from_PeroxyRadical')
        rxn_4 = ARCReaction(r_species=[ARCSpecies(label='C2H4O', smiles='CC=O'),
                                       ARCSpecies(label='HO2', smiles='O[O]')],
                            p_species=[ARCSpecies(label='C2H5O3', smiles='CC(O)O[O]')])
        rxn_4.check_attributes()
        rxn_4.determine_family(rmg_database=self.rmgdb)
        self.assertEqual(rxn_4.family.label, 'HO2_Elimination_from_PeroxyRadical')
    #
...
#         self.assertEqual(reactants_xyz_dict, expected_reactants_xyz_dict)
        ...
        self.assertEqual(rxn.atom_map[:5], [0, 1, 2, 8, 7])
        self.assertIn(rxn.atom_map[5], [4, 5])
        self.assertIn(rxn.atom_map[6], [3, 5])
        self.assertIn(rxn.atom_map[7], [3, 4])
        self.assertEqual(rxn.atom_map[8:], [6, 9])
        self.assertTrue(check_atom_map(rxn))

        rxn = ARCReaction(r_species=[p_1, p_2], p_species=[r_1])
        self.assertEqual(rxn.atom_map[:3], [0, 1, 2])
        self.assertIn(rxn.atom_map[3], [6, 7])
        self.assertIn(rxn.atom_map[4], [5, 7])
        self.assertIn(rxn.atom_map[5], [5, 6])
        self.assertEqual(rxn.atom_map[6:], [8, 4, 3, 9])
        self.assertTrue(check_atom_map(rxn))
