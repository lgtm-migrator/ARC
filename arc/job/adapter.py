...
class JobAdapter(ABC):
    ...
    def _determine_workers(self):
        ...
        if self.workers is None:
            # A trend line for the desired number of workers vs. the number of processes:
            #    y    = 1.7 *     x     ** 0.35
            # workers = 1.7 * processes ** 0.35
            # gives the following output (processes -> workers): 10 -> 4, 100 -> 9, 1000 -> 19, 1e4 -> 43, 1e5 -> 96
            # We cap the number of workers at 100.
            # Use just 1 or 2 workers if there are less than 10 processes.
            if self.number_of_processes <= tasks_coeff['max_one']:
                self.workers = 1
            elif self.number_of_processes <= tasks_coeff['max_two']:
                self.workers = 2
            else:
                self.workers = min(math.ceil(tasks_coeff['A'] * self.number_of_processes ** tasks_coeff['b']),
                                   tasks_coeff['cap'])
