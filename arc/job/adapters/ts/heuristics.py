...
class HeuristicsAdapter(JobAdapter):
    ...
    def __init__(self,
        ...
        if dihedral_increment is not None and dihedral_increment < 0 or dihedral_increment > 360:
            raise ValueError(f'Got an illegal value for dihedral_increment: {dihedral_increment}')
        ...
        for j, param in enumerate(coords):
            if param is not None:
                if i == 0:
                    ...
                    # Atom E should refer to atom D.
                    new_param = f'R_{num_atoms_1 + 2}_{num_atoms_1 + 1}'
                elif i == 2 and j == 1:
                    new_param = f'A_{num_atoms_1 + 2}_{num_atoms_1 + 1}_{num_atoms_1}'
                elif i == 2 and j == 2:
                    new_param = f'D_{num_atoms_1 + 2}_{num_atoms_1 + 1}_{num_atoms_1}_{h1}'
                else:
